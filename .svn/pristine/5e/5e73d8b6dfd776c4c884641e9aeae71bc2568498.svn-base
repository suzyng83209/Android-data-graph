 ...
 *
 * Modified on 5/28/17 by Dan Timonera.
 * 
 * @author Kirill 
 *
 */
public class LineGraphView extends View
{
	 ...
	 * Modified on 5/28/17 by Dan Timonera.
	 *
	 * Modified to require external specification of graph dimensions based on screen resolution.
     * Font size increased for readability; https://stackoverflow.com/questions/12166476/android-canvas-drawtext-set-font-size-from-width.
	 *
	 ...
	 *
	 * @param width The width of the space to the right of the vertical axis.
	 * @param height The height of the line graph.
	 * @param axisWidthPercentage The width of the space to the left of the vertical axis as a percentage of the width.
	 */
	public LineGraphView(Context context, int dataWidth, List<String> labels, int width, int height, float axisWidthPercentage) {
		super(context);
		...
		// graph dimensions initialization by Dan Timonera

		WIDTH = width;
		HEIGHT = height;
		AXIS_WIDTH = (int) (width * axisWidthPercentage);

        graphPaint.setTextSize(20f);
	}
     ...
     * Modified on 5/28/17 by Dan Timonera.
     *
     * Changed label spacing to accommodate increased font size.
     *
	...
	private void prepForData(Canvas canvas)
	{
		...
		canvas.drawText(Float.toString(maxY) + " m/s^2", 0, 20, graphPaint);
		canvas.drawText("-" + Float.toString(maxY) + " m/s^2", 0, HEIGHT - 5, graphPaint);
		
		for(int i = 0; i < labels.size(); i++){
            int basePosition = i * 30;
            int textPosition = 50 + basePosition;
            int linePosition = 10 + textPosition;

			canvas.drawText(labels.get(i) + ":", 0, textPosition, graphPaint);
			canvas.drawLine(0, linePosition, AXIS_WIDTH - 20, linePosition, linePaints.get(i));
		}
	}
